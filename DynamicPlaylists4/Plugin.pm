...
sub postinitPlugin {
	...
	if ($material_enabled && Plugins::MaterialSkin::Plugin->can('registerHomeExtra')) {
		eval {
			require Plugins::DynamicPlaylists4::HomeExtras;
		};
		$log->error("Could not load DPL Home Extras: $@") if $@;
		if (main::WEBUI) {
			require Plugins::DynamicPlaylists4::Settings::HomeExtras;
			Plugins::DynamicPlaylists4::Settings::HomeExtras->new();
		}
	}
}
			...
			for my $item (keys %{$items}) {
				...
				if (!$playlist->{'playlistcategory'}) {
					if ($playlist->{'menulisttype'} && ($playlist->{'menulisttype'} eq 'contextmenu')) {
						...
						unless ($playlist->{'menulisttype'} && ($playlist->{'menulisttype'} eq 'homeextrasmenu')) {
				...
				if (scalar(@{$groups}) > 0) {
					for my $currentgroups (@{$groups}) {
						...
						for my $group (@{$currentgroups}) {
							next if $group && $group eq 'Home extras menus';
...
sub handleWebMixParameters {
	...
	while (defined($params->{'dynamicplaylist_parameter_'.$i})) {
		...
		} elsif ($parameter && $parameter->{'type'} && ($parameter->{'type'} eq 'albumtitlecontains' || $parameter->{'type'} eq 'tracktitlecontains')) {
			my $searchTerm = $params->{'dynamicplaylist_parameter_'.$i};
			$searchTerm = handleSearchText($searchTerm);
			%value = (
				'id' => $searchTerm
			);
			main::DEBUGLOG && $log->is_debug && $log->debug('using this string search for LMS titlesearch: '.Data::Dump::dump($params->{'dynamicplaylist_parameter_'.$i}));
...
sub cliJiveHandler {
	...
	foreach my $item (@{$menuResult}) {
		if ($item->{'dynamicplaylistenabled'} && (!defined($item->{'menulisttype'}) || ($item->{'menulisttype'} ne 'contextmenu') && $item->{'menulisttype'} ne 'homeextrasmenu')) {
			my $name = $item->{'name'};
...
sub parseContent {
	...
	if ($content) {
		...
		for my $line (@playlistDataArray) {
			...
			my $returnedItemType = parseItemType($line);
...
sub parseItemType {
	my $line = shift;
	if ($line =~ /^\s*--\s*ItemType\s*[:=]\s*/) {
		$line =~ m/^\s*--\s*ItemType\s*[:=]\s*([^:]+)\s*(.*)$/;
		my $itemType = $1;
		...
		if ($itemType) {
			...
			$log->warn("No value or error in ItemType: $line");
			$log->warn('Option values: ItemType = '.Data::Dump::dump($itemType));
			return undef;
		}
	}
	return undef;
...
sub parseNoVFD {
	my $line = shift;
	if ($line =~ /^\s*--\s*PlaylistNoVFD\s*[:=]\s*/) {
		$line =~ m/^\s*--\s*PlaylistNoVFD\s*[:=]\s*([^:]+)\s*(.*)$/;
		my $novfd = $1;
		...
		if ($novfd) {
			...
			main::DEBUGLOG && $log->is_debug && $log->debug("No value or error in novfd: $line");
			main::DEBUGLOG && $log->is_debug && $log->debug('Option values: novfd = '.Data::Dump::dump($novfd));
			return undef;
		}
	}
	return undef;
...
sub getHomeExtraMenuItems {
	my ($client, $cb, $args) = @_;
	if (!$client) {
		$cb->({
			items => [{
				name => string('PLUGIN_DYNAMICPLAYLISTS4_NO_PLAYER'),
				type => 'text'
			}]
		});
		return;
