-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_GENRES_PLAYEDLONGAGO
...
create temporary table dynamicplaylist_random_genres as
	select genre_track.genre as genre from genre_track
		...
		group by genre_track.genre
			having ((strftime('%s',DATE('NOW','-'PlaylistPeriodPlayedLongAgo' YEAR'))-max(ifnull(tracks_persistent.lastPlayed,0))) > 0)
		order by random()
		limit 1;
