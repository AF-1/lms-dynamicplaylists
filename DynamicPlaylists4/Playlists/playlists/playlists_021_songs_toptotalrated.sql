-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_PLAYLISTS_TOPRATEDTOTAL
...
create temporary table dynamicplaylist_random_playlists as
	select toptotalrated.playlist as playlist from
		(select playlist_track.playlist as playlist, sum(t.rating) as totalrating from playlist_track
		join (
			select tracks.id, tracks.url, tracks.urlmd5,
			...
			group by tracks.id
		) as t on t.url = playlist_track.track
