-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_PLAYLISTS_PLAYEDLONGAGO
...
create temporary table dynamicplaylist_random_playlists as
	select playlist_track.playlist as playlist from playlist_track
		...
		group by playlist_track.playlist
			having ((strftime('%s',DATE('NOW','-'PlaylistPeriodPlayedLongAgo' YEAR'))-ifnull(tracks_persistent.lastPlayed,0)) > 0)
		order by random()
		limit 1;
