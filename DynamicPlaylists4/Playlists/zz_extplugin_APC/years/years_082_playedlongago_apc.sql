-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_YEARS_PLAYEDLONGAGO_APC
...
create temporary table dynamicplaylist_random_years as
	select tracks.year as year from tracks
		...
		having max(ifnull(alternativeplaycount.lastPlayed,0)) > 0 and ((strftime('%s',DATE('NOW','-'PlaylistPeriodPlayedLongAgo' YEAR'))-max(ifnull(alternativeplaycount.lastPlayed,0))) > 0)
		order by random()
		limit 1;
