-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_ARTISTS_LEASTPLAYED_APC
...
create temporary table dynamicplaylist_random_contributors as
	select leastplayed.contributor as contributor from
		(select ct.contributor as contributor, sum(t.playcount) as sumcount, count(distinct t.id) as totaltrackcount
		from (
