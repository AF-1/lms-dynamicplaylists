...
create temporary table dynamicplaylist_random_works as
	select mostplayed.album as album, mostplayed.work as work, mostplayed.grouping as grouping from
		(select tracks.album as album, tracks.work as work, tracks.grouping as grouping, sum(ifnull(alternativeplaycount.playCount,0)) as sumcount, count(distinct tracks.id) as totaltrackcount from tracks
