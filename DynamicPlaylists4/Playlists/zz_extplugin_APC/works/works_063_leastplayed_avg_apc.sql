...
create temporary table dynamicplaylist_random_works as
	select leastavgplayed.album as album, leastavgplayed.work as work, leastavgplayed.performance as performance from
		(select tracks.album as album, tracks.work as work, tracks.performance as performance, avg(ifnull(alternativeplaycount.playCount,0)) as avgcount, count(distinct tracks.id) as totaltrackcount from tracks
