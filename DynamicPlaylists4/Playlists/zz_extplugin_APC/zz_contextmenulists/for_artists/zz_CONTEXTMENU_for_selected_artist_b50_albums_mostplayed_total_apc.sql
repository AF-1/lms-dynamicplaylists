-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_CONTEXT_ARTIST_ALBUMS_MOSTPLAYED_APC
...
create temporary table dynamicplaylist_random_albums as
	select tracks.album as album, sum(ifnull(alternativeplaycount.playCount,0))/count(distinct contributor_track.role) as sumcount, count(distinct tracks.id) as totaltrackcount from tracks
