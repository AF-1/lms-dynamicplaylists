...
select distinct tracks.id, tracks.primary_artist from tracks
	...
	join alternativeplaycount on alternativeplaycount.urlmd5 = tracks.urlmd5 and ifnull(alternativeplaycount.lastPlayed,0) > 0 and ((strftime('%s',DATE('NOW','-'PlaylistPeriodPlayedLongAgo' YEAR'))-ifnull(alternativeplaycount.lastPlayed,0)) > 0)
