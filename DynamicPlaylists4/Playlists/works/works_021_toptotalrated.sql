-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_WORKS_TOPRATEDTOTAL
...
create temporary table dynamicplaylist_random_works as
	select toptotalrated.album as album, toptotalrated.work as work, toptotalrated.performance as performance from
		(select tracks.album as album, tracks.work as work, tracks.performance as performance, sum(ifnull(tracks_persistent.rating,0)) as totalrating, count(distinct tracks.id) as totaltrackcount from tracks
