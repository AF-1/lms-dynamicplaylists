-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_WORKS_NEVERPLAYED
...
create temporary table dynamicplaylist_random_works as
	select tracks.album as album, tracks.work as work, tracks.performance as performance, sum(ifnull(tracks_persistent.playCount,0)) as sumplaycount, count(distinct tracks.id) as totaltrackcount from tracks
		...
		where
			...
			and case when tracks.performance is not null then tracks.performance else tracks.work end
				case
