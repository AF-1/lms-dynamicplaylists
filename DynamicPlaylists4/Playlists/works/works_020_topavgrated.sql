-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_WORKS_TOPRATEDAVG
...
create temporary table dynamicplaylist_random_works as
	select topavgrated.album as album, topavgrated.work as work, topavgrated.grouping as grouping from
		(select tracks.album as album, tracks.work as work, tracks.grouping as grouping, avg(ifnull(tracks_persistent.rating,0)) as avgrating, count(distinct tracks.id) as totaltrackcount from tracks
