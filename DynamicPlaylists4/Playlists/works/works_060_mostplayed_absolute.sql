-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_WORKS_MOSTPLAYED
...
create temporary table dynamicplaylist_random_works as
	select mostplayed.album as album, mostplayed.work as work, mostplayed.performance as performance from
		(select tracks.album as album, tracks.work as work, tracks.performance as performance, sum(ifnull(tracks_persistent.playCount,0)) as sumcount, count(distinct tracks.id) as totaltrackcount from tracks
