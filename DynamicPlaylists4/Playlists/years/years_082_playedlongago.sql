-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_YEARS_PLAYEDLONGAGO
...
create temporary table dynamicplaylist_random_years as
	select tracks.year as year from tracks
		...
		having max(ifnull(tracks_persistent.lastPlayed,0)) > 0 and ((strftime('%s',DATE('NOW','-'PlaylistPeriodPlayedLongAgo' YEAR'))-max(ifnull(tracks_persistent.lastPlayed,0))) > 0)
		order by random()
		limit 1;
