-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_CONTEXT_GENRE_ARTISTS_TOPRATEDTOTAL
...
create temporary table dynamicplaylist_random_contributors as
	select toptotalrated.contributor as contributor from
		(select t.contributor as contributor, sum(t.rating) as totalrating, count(distinct t.id) as totaltrackcount
		from (
