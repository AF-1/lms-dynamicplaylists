-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS4_BUILTIN_PLAYLIST_ARTISTS_TOPRATEDTOTAL
...
create temporary table dynamicplaylist_random_contributors as
	select toptotalrated.contributor as contributor from
		(select ct.contributor as contributor, sum(t.rating) as totalrating, count(distinct t.id) as totaltrackcount
		from (
