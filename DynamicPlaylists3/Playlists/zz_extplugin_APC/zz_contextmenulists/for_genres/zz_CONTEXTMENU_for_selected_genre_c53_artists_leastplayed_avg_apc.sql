-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS3_BUILTIN_PLAYLIST_CONTEXT_GENRE_ARTISTS_LEASTPLAYEDAVG_APC
...
create temporary table dynamicplaylist_random_contributors as
	select leastavgplayed.contributor as contributor from
		(select distinct contributor_track.contributor as contributor, avg(ifnull(alternativeplaycount.playCount,0)) as avgcount, count(distinct tracks.id) as totaltrackcount from tracks
