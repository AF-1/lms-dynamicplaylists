-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS3_BUILTIN_PLAYLIST_CONTEXT_GENRE_ARTISTS_LEASTPLAYED_APC
...
create temporary table dynamicplaylist_random_contributors as
	select leastplayed.contributor as contributor from
		(select distinct contributor_track.contributor as contributor, sum(ifnull(alternativeplaycount.playCount,0)) as sumcount, count(distinct tracks.id) as totaltrackcount from tracks
