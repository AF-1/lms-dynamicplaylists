-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS3_BUILTIN_PLAYLIST_CONTEXT_DECADE_ALBUMS_LEASTPLAYED_APC
...
create temporary table dynamicplaylist_random_albums as
	select leastplayed.album as album from
		(select distinct tracks.album as album, sum(ifnull(alternativeplaycount.playCount,0)) as sumcount, count(distinct tracks.id) as totaltrackcount from tracks
