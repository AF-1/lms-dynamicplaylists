...
create temporary table dynamicplaylist_random_genres as
	select genre_track.genre as genre from genre_track
		...
		where
			...
			and
				case
					when 'PlaylistParameter1'=1 then ((strftime('%s',DATE('NOW','-'PlaylistPeriodRecentlyPlayed' DAY'))-ifnull(alternativeplaycount.lastPlayed,0)) < 0)
					else 1
				end
			and
				case
		...
		group by genre_track.genre
			having
					case
						when 'PlaylistParameter1'=2 then ((strftime('%s',DATE('NOW','-'PlaylistPeriodRecentlyPlayed' DAY'))-max(ifnull(alternativeplaycount.lastPlayed,0))) > 0)
						else 1
					end
		order by random()
		limit 1;
