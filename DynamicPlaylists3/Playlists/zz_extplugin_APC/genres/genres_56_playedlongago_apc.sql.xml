-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS3_BUILTIN_PLAYLIST_GENRES_PLAYEDLONGAGO_APC
...
create temporary table dynamicplaylist_random_genres as
	select genre_track.genre as genre from genre_track
		...
		group by genre_track.genre
			having ((strftime('%s',DATE('NOW','-'PlaylistPeriodPlayedLongAgo' YEAR'))-max(ifnull(alternativeplaycount.lastPlayed,0))) > 0)
		order by random()
		limit 1;
