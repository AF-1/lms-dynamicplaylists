-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS3_BUILTIN_PLAYLIST_CONTEXT_GENRE_ARTISTS_TOPRATEDAVG
...
create temporary table dynamicplaylist_random_contributors as
	select topavgrated.contributor as contributor from
		(select distinct contributor_track.contributor as contributor, avg(ifnull(tracks_persistent.rating,0)) as avgrating, count(distinct tracks.id) as totaltrackcount from tracks
