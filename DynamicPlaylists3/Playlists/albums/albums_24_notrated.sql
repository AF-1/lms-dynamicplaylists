-- PlaylistName:PLUGIN_DYNAMICPLAYLISTS3_BUILTIN_PLAYLIST_ALBUMS_UNRATED
...
create temporary table dynamicplaylist_random_albums as
	select notrated.album as album, notrated.sumrating as sumrating from
		(select distinct tracks.album as album, sum(ifnull(tracks_persistent.rating,0)) as sumrating, count(distinct tracks.id) as totaltrackcount from tracks
			join albums on
